...
## Reglas de este repo (leer CLAUDE.md antes de tocar nada)

- `src/domain/**`, `src/lib/**`, `src/app/actions/**`, checkout y `/admin` son maquinaria: plata entera en guaraníes, nunca float; `orders.status` sólo cambia por `transitionOrder`.
- Toda resta entre columnas `UNSIGNED` en SQL va con `CAST(... AS SIGNED)` (KNOWN-ISSUES.md).
- Cambios de schema van con `pnpm db:generate` y la migración commiteada.
- Antes de reportar: `pnpm typecheck && pnpm lint && pnpm test` (los tests de integración necesitan `docker compose up -d` y `TEST_DATABASE_URL`).
- No le saques `data-testid` a nada. No hardcodees el nombre de la tienda fuera de `src/config/tienda.ts`.
